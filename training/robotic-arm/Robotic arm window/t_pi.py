...
class ArmTrackingSystem:
    def __init__(self, camera_index=1, model_path="best.pt", use_distance_sensor=False, 
    ...
    def _tracking_loop(self):
        ...
        while self.is_tracking:
            ...
            if self.visualization and frame is not None:
                cv2.imshow("ssgz", frame)
                
                # 按ESC键退出
                key = cv2.waitKey(1) & 0xFF
                if key == 27:
                    #self.stop_tracking()
                    self.is_tracking = False  # 仅停止循环，不调用 stop_tracking()
    ...
    def calibrate(self):
        ...
        print("请将标准物体（如A4纸）放在摄像头前方约50厘米处...")
        
        while True:
            target_coords, detection_info, frame = self.detector.detect_and_track()
            
            if frame is not None:
                cv2.putText(frame, "c_accurate_esc", (10, 60), #按C键校准，ESC退出
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.imshow("accurate", frame)
            ...
            if key == 27:  # ESC
                ...
                self.detector.calibrate_pixel_ratio(50.0, known_width, detected_width) #这里可以调节距离参数
                print("校准完成！")
                time.sleep(1)
                break
    ...
    def release(self):
        ...
        cv2.destroyAllWindows()
        # 确保摄像头资源释放
        if hasattr(self.detector, 'picam2'):
            self.detector.picam2.stop()
            self.detector.picam2.close()
