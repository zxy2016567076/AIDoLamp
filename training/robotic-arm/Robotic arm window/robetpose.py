...
def calculate_ik(target_pos, distance=40, lamp_mode="book"):
    ...
    if lamp_mode == "book":
        ...
        # 互动模式：灯应该直接朝向人脸，位置可以更自然
        # 默认高度，不需要过多考虑美观性，只需确保摄像头能看到人脸
        x_ee = x_target * 0.9  # 略微向基座靠近
        y_ee = y_target * 0.9
        z_ee = z_target + height_offset
    ...
    # 计算腕部角度 - 使灯罩始终朝向目标
    # 注意：腕部0度表示与z轴重合，我们需要计算使末端执行器指向目标的角度
    ...
    # 计算前臂方向在base_angle平面内的投影
    forearm_dir_proj = np.array([np.cos(forearm_angle), np.sin(forearm_angle)])
    
    # 计算目标方向在base_angle平面内的投影
    target_dir_proj = np.array([
        np.sqrt(wrist_to_target[0]**2 + wrist_to_target[1]**2),
        wrist_to_target[2]
    ])
    if np.linalg.norm(target_dir_proj) > 0:
        target_dir_proj = target_dir_proj / np.linalg.norm(target_dir_proj)
    
    # 计算两个方向向量之间的角度
    wrist_angle = np.arctan2(target_dir_proj[1], target_dir_proj[0]) - np.arctan2(forearm_dir_proj[1], forearm_dir_proj[0])
    
    # 调整角度到0-2π区间
    wrist_angle = wrist_angle % (2 * np.pi)
...
def forward_kinematics(base_angle, shoulder_angle, elbow_angle, wrist_angle):
    ...
    # 计算肘部位置 - 底座0度对应x轴正方向，肩部0度对应z轴正方向
