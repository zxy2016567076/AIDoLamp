...
# 关节角度限制（以弧度计）
BASE_LIMITS = (-np.pi * 135/180, np.pi * 135/180)     # 底座可以向y轴正负方向分别转动135度
SHOULDER_LIMITS = (-np.pi * 90/180, np.pi * 90/180)   # 肩部可以向x轴正负方向分别转动90度
ELBOW_LIMITS = (0, np.pi * 150/180)                   # 肘部可以向x轴正方向转动150度
WRIST_LIMITS = (0, np.pi * 130/180)                   # 腕部可以向x轴正方向转动130度

def calculate_ik(target_pos, distance=40, lamp_mode="book"):
    """
    计算四轴机械臂的逆运动学，使台灯以合适的角度朝向目标点。
    
    参数:
    target_pos -- 目标位置的元组 (x, y, z)
    distance -- 末端执行器与目标之间的期望距离（厘米）
    lamp_mode -- 台灯模式，"book"表示工作模式照书，"face"表示互动模式跟踪人脸
    ...
    if lamp_mode == "book":
        # 工作模式：确保光照效果和护眼
        # 45cm是典型的阅读距离，提供舒适照明而不会太亮刺眼
        target_distance = min(max(distance, 40), 50)  # 限制在40-50cm之间
        
        # 书本通常在桌面上，需要一定高度来提供良好的光照角度（约30-40度）
        height_offset = 30  # 灯相对于书本的高度
    else:  # face模式
        # 互动模式：保持一个适合交互的距离
        target_distance = distance  # 互动模式不限制距离
        height_offset = 20  # 对话时保持自然的高度
    ...
    if lamp_mode == "book":
        ...
        # 灯的位置应该在目标点上方，并稍微向基座方向偏移，形成一个普通台灯的姿态
        # 水平偏移量随距离增加而增加
        ...
        # 计算腕部位置 - 保持自然距离
        wrist_pos = np.array([
            x_target - 0.2 * target_xy_distance * base_to_target_dir[0],
            y_target - 0.2 * target_xy_distance * base_to_target_dir[1],
            z_target + 0.5 * height_offset
        ])
        
        # 计算腕部到目标的方向向量
        direction_to_target = target_vec - wrist_pos
        if np.linalg.norm(direction_to_target) > 0:
            direction_to_target = direction_to_target / np.linalg.norm(direction_to_target)
            
        # 计算末端执行器位置
        end_effector_pos = wrist_pos + L4 * direction_to_target

        # 直接使用end_effector_pos更新坐标（删除后续对x_ee/y_ee/z_ee的赋值）
        x_ee, y_ee, z_ee = end_effector_pos  # 新增此行，从end_effector_pos中提取坐标
    ...
    if current_distance > 0:
        # 按比例调整位置
        ratio = target_distance / current_distance # 计算缩放比例
    ...
    # 检查点是否可达
    if d > L2 + L3:
        # 如果超出最大距离，则沿当前方向缩短至最大可达距离
        direction_to_wrist = np.array([r, z])
        direction_to_wrist = direction_to_wrist / np.linalg.norm(direction_to_wrist)
        
        # 调整为最大可达距离（留有余量）
        max_d = L2 + L3 - 0.1
        r = direction_to_wrist[0] * max_d
        z = direction_to_wrist[1] * max_d
        
        # 更新距离
        d = max_d
    
    if d < abs(L2 - L3):
        # 如果小于最小距离，则沿当前方向延长至最小可达距离
        direction_to_wrist = np.array([r, z])
        if np.linalg.norm(direction_to_wrist) > 0:
            ...
            # 调整为最小可达距离（留有余量）
            min_d = abs(L2 - L3) + 0.1
            r = direction_to_wrist[0] * min_d
            z = direction_to_wrist[1] * min_d
    ...
    # 转换为机械臂约定的肘部角度 - 肘部0度表示与z轴重合
    elbow_angle = np.pi - elbow_angle
    
    # 确保肘部角度在限制范围内
    if elbow_angle < ELBOW_LIMITS[0]:
        elbow_angle = ELBOW_LIMITS[0]
    elif elbow_angle > ELBOW_LIMITS[1]:
        elbow_angle = ELBOW_LIMITS[1]
    ...
    # 确保肩部角度在限制范围内
    if shoulder_angle < SHOULDER_LIMITS[0]:
        shoulder_angle = SHOULDER_LIMITS[0]
    elif shoulder_angle > SHOULDER_LIMITS[1]:
        shoulder_angle = SHOULDER_LIMITS[1]
    ...
    if np.linalg.norm(wrist_to_target) > 0:
    ...
    # 计算腕部旋转角度使末端执行器指向目标
    cross_prod = np.cross(forearm_dir, wrist_to_target)
    dot_prod = np.dot(forearm_dir, wrist_to_target)
    wrist_angle = np.arctan2(np.linalg.norm(cross_prod), dot_prod)
    
    # 确保腕部角度在限制范围内
    
    # 确保腕部角度在限制范围内
    if wrist_angle < WRIST_LIMITS[0]:
        wrist_angle = WRIST_LIMITS[0]
    elif wrist_angle > WRIST_LIMITS[1]:
        wrist_angle = WRIST_LIMITS[1]
...
# 测试函数
def test_arm_kinematics(target_position=(40, 0, 0), mode="book"):
    """
    测试机械臂运动学计算
    ...
    """
    # 设置合适的距离
    optimal_distance = 45 if mode == "book" else 40
    
    # 计算逆运动学
    base_angle, shoulder_angle, elbow_angle, wrist_angle, end_pos = calculate_ik(
        target_position, distance=optimal_distance, lamp_mode=mode
    )
    
    # 计算腕部位置（通过逆运动学逻辑）
    direction_to_target = (np.array(target_position) - end_pos) / np.linalg.norm(np.array(target_position) - end_pos)
    wrist_pos = end_pos - L4 * direction_to_target
    
    # # 计算关节位置
    # joint_positions = forward_kinematics(base_angle, shoulder_angle, elbow_angle, wrist_angle)
    joint_positions = forward_kinematics(base_angle, shoulder_angle, elbow_angle, wrist_angle)
    joint_positions[-1] = end_pos  # 新增此行，替换末端位置
    
    # 计算角度（度数）
    angles_deg = angles_to_degrees(base_angle, shoulder_angle, elbow_angle, wrist_angle)
    
    # 显示结果
    print(f"\n=== 目标位置: {target_position} ({mode}模式) ===")
    print(f"灯头位置: ({end_pos[0]:.1f}, {end_pos[1]:.1f}, {end_pos[2]:.1f})")
    print(f"目标到灯头距离: {np.linalg.norm(np.array(target_position) - end_pos):.1f} cm")
    print(f"关节角度（度）:")
    for joint, angle in angles_deg.items():
        print(f"  {joint}: {angle:.1f}°")
    
    # 可视化
    title = f"台灯{mode}模式 @ {target_position}"
...
if __name__ == "__main__":
    ...
    for pos in face_positions:
        test_arm_kinematics(pos, "face")
    
    # 测试平滑轨迹生成
    print("\n===== 测试平滑轨迹 =====")
    start = (0, 0, 0, 0)  # 起始角度
    end = (np.pi/4, np.pi/4, np.pi/4, np.pi/4)  # 目标角度
