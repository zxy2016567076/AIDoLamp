...
class YOLODetector:
    def __init__(self, model_path="best.pt", camera_index=1):
        """
        初始化YOLO检测器，支持选择性检测人脸或书本
        
        参数:
        model_path -- YOLOv8模型路径，默认使用best.pt
    ...
    def detect(self, frame, conf_threshold=0.5, target_class=None):
        """
        通用检测方法，可以通过target_class指定仅检测特定类别
        
        参数:
        frame -- 输入图像帧
        conf_threshold -- 置信度阈值
        target_class -- 目标类别，可以是'face'或'book'，如果为None则检测所有类别
        
        返回:
        results_dict -- 检测结果字典，键为类别名称，值为[x, y, w, h]格式的坐标
        """
    ...
    def convert_to_face_coordinates(self, x_img, y_img, width, height):
        """
        将YOLO检测的人脸坐标转换为face.py所需的坐标系
        
        参数:
        x_img, y_img -- 图像中的目标中心坐标（像素）
        width, height -- 检测框宽度和高度（像素）
        
        返回:
        x, y, z -- face.py所需的坐标（厘米）
        """
    ...
    def detect_face(self, frame=None, draw_results=False):
        """
        仅检测人脸并返回face.py所需的坐标
        
        参数:
        frame -- 输入图像帧，若为None则自动捕获一帧
        draw_results -- 是否在图像上绘制检测结果
        
        返回:
        face_coordinates -- 转换后的人脸坐标 (x, y, z)
        annotated_frame -- 绘制了检测结果的图像（如果draw_results为True）
        """
        if frame is None:
    ...
    def detect_book(self, frame=None, draw_results=False):
        ...
        返回:
        book_coordinates -- 转换后的书本坐标 (x, y, z)
        annotated_frame -- 绘制了检测结果的图像（如果draw_results为True）
        """
        if frame is None:
            frame = self.picam2.capture_array()
