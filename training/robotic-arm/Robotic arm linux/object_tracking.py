...
class ObjectTracker:
    ...
    def face_forward_kinematics(self, base_angle, wrist_angle):
        ...
        返回:
        joint_positions -- 各关节坐标列表 [底座, 肩部, 肘部, 腕部, 末端]
        """
        # 固定肩部和肘部角度
        shoulder_angle = FIXED_SHOULDER_ANGLE
        elbow_angle = FIXED_ELBOW_ANGLE
    ...
    def track_face(self, face_coordinates):
        ...
        参数:
        face_coordinates -- 人脸的三维坐标 (x, y, z)
        
        返回:
        base_angle, wrist_angle -- 底座和腕部舵机角度（弧度）
        """
        base_angle, wrist_angle, end_effector_pos = self.calculate_face_ik(face_coordinates)
        
        # 可视化（调试模式下使用）
        # joint_positions = self.face_forward_kinematics(base_angle, wrist_angle)
        # self.plot_robotic_arm(joint_positions, target_pos=face_coordinates, 
        #                     title="人脸跟踪模式")
    ...
    def track_book(self, book_coordinates, distance=45):
        ...
        参数:
        book_coordinates -- 书本的三维坐标 (x, y, z)
        distance -- 末端执行器与书本的目标距离，默认45厘米
        
        返回:
        base_angle, shoulder_angle, elbow_angle, wrist_angle -- 四个舵机角度（弧度）
        """
        ...
        # 可视化（调试模式下使用）
        # joint_positions = self.book_forward_kinematics(
        #     base_angle, shoulder_angle, elbow_angle, wrist_angle, end_effector_pos
        # )
        # self.plot_robotic_arm(joint_positions, target_pos=book_coordinates, 
        #                     end_effector_pos=end_effector_pos, title="书本跟踪模式")
